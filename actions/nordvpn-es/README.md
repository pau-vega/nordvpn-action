# NordVPN Spanish Egress (composite action)

Connects the current GitHub Actions runner to a NordVPN Spanish exit node, verifies `country=ES` against two independent geo providers, and exposes a diagnostics bundle as step outputs. Intended to be consumed from any repo via `uses:`. Teardown lives in the sibling `disconnect/` sub-action.
...
2. **Connect (bounded retry)** — up to 2 attempts: writes service credentials to `$RUNNER_TEMP/nordvpn-auth.txt` at 0600, starts `openvpn --daemon` against the bundled `nordvpn-es.ovpn`, polls `ip -4 addr show tun0` for an assigned IPv4 address (30s timeout, 2s interval).
3. **Verify** — queries `ipinfo.io/json` (primary) and `ifconfig.co/json` (secondary). Both must independently return `ES`. Emits the six-field diagnostics bundle above to `$GITHUB_OUTPUT`.
...
3. **Verify with the next workflow run.** Open (or re-run) any pull request whose
   ...
   `::notice::` annotation and confirm `country: ES` plus a fresh `exit-ip`.
...
| Exact version tag | `@nordvpn-es-v1.0.1` | You want a specific, frozen version without the SHA noise. Produced by [release-please](https://github.com/googleapis/release-please) on every automated release. |
| Floating major tag | `@nordvpn-es-v1` | You want automatic non-breaking updates on every `v1.x.y` release. Force-moved by the CI on every new `nordvpn-es-v1.x.y` release. |
...
Automated releases are cut by [release-please](https://github.com/googleapis/release-please) whenever a conventional commit lands under `actions/nordvpn-es/**`. The resulting tag is `nordvpn-es-vX.Y.Z` and the floating `nordvpn-es-v1` tag is force-updated to the same commit.
...
The `nordvpn-es-v1` tag is force-moved on every release. For reproducibility, pin SHA.
...
- **Country mismatch (`country != ES`) in the VPN diagnostics table.** The
