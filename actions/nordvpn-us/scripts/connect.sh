...
# .github/vpn/nordvpn-us.ovpn, and polls `ip -4 addr show tun0` until an IPv4 is assigned
...
# Ubuntu-only guard (NVUS-06, AGENTS.md Security section)
...
CONFIG_FILE="${GITHUB_ACTION_PATH}/vpn/nordvpn-us.ovpn"
...
# Resolve a currently-online US openvpn_tcp server via NordVPN's public
...
# United States; filters by openvpn_tcp technology; limit=1 returns the lowest-load
...
NORD_QUERY='filters%5Bcountry_id%5D=228&filters%5Bservers_technologies%5D%5Bidentifier%5D=openvpn_tcp&limit=1'
...
if [[ -z "$NORDVPN_REMOTE_IP" || "$NORDVPN_REMOTE_IP" == "null" ]]; then
  echo "::error::api.nordvpn.com returned empty hostname for US openvpn_tcp"
  exit 1
fi
