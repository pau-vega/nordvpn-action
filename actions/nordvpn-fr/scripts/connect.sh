...
# .github/vpn/nordvpn-fr.ovpn, and polls `ip -4 addr show tun0` until an IPv4 is assigned
...
# Ubuntu-only guard (NVFR-06, AGENTS.md Security section)
...
CONFIG_FILE="${GITHUB_ACTION_PATH}/vpn/nordvpn-fr.ovpn"
...
# Resolve a currently-online French openvpn_tcp server via NordVPN's public
# recommendations API. The original design assumed DNS round-robin at
# `fr.nordvpn.com`, but that hostname does not exist as a DNS record — only
# server-specific names like `frNNN.nordvpn.com` resolve. country_id=74 is
# France; filters by openvpn_tcp technology; limit=1 returns the lowest-load
...
NORD_QUERY='filters%5Bcountry_id%5D=74&filters%5Bservers_technologies%5D%5Bidentifier%5D=openvpn_tcp&limit=1'
if ! NORDVPN_REMOTE_IP=$(curl -fsS --max-time 10 "${NORD_API}?${NORD_QUERY}" | jq -r '.[0].station'); then
  echo "::error::failed to query api.nordvpn.com for recommended FR server"
  exit 1
fi
if [[ -z "$NORDVPN_REMOTE_IP" || "$NORDVPN_REMOTE_IP" == "null" ]]; then
  echo "::error::api.nordvpn.com returned empty hostname for FR openvpn_tcp"
  exit 1
fi
echo "[connect.sh] NordVPN server IP: $NORDVPN_REMOTE_IP"
